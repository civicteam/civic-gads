...
## Metadata
- **Version:** 2.3.0-civic.1
- **Optimized for:** Machine Comprehension
- **Runtime:** Python 3.x
- **Host:** Claude Code (ported from Gemini CLI; see NOTICE)

@conversions/CLAUDE.md
...
- **NO PERSISTENCE:** Never save the confirmed API version to Claude Code's auto-memory system. The API version is per-session state and must NOT be written to any file under `~/.claude/projects/*/memory/`.
- **READ-ONLY:** Only execute `search`, `search_stream`, or `get` methods.
- **SURGICAL UPDATES:** When modifying files, use the `Edit` tool with minimal context to avoid unintended regressions.
...
- **SOURCE OF TRUTH:** Never rely solely on high-level documentation summaries or search snippets for API capabilities. Always use `Grep` and `Read` to verify the literal `.proto` definitions or Python client library docstrings before concluding an API feature's behavior or requirements. When searching for client library definitions or examples, you MUST prioritize the local `client_libs/` directory (e.g., `client_libs/google-ads-python/`) and NEVER use system-wide paths (e.g., `/usr/local/lib/` or `~/.pyenv/`) to avoid version mismatches and environment-specific discrepancies.
...
- **EXPLICIT LOGGING:** Before calling `Bash` for Python or any API search tool, you MUST explicitly state which protocol step you are currently executing (e.g., "Protocol 3.2: Executing Ruff linting on /tmp/script.py").
...
1.  **Search (Exact):** `WebSearch` with query `google ads api release notes`.
2.  **Fetch (Source):** `WebFetch` content from `developers.google.com/google-ads/api/docs/release-notes`.
...
If the `WebFetch` tool is unavailable and you cannot complete the standard validation workflow in section 1.3, you MUST use the following fallback procedure:
1.  **SEARCH:** Use `WebSearch` with the query: `google ads api release notes`.
...
- **Write:** Use `Write` for new scripts.
- **Modify:** Use `Edit` for surgical updates.
...
4.  **Finalize:** Use the fixed code in the `Write` or `Bash` tool.
...
#### 5.1. Conversions
- **Mandatory Path:** Follow `conversions/CLAUDE.md` workflow (auto-imported above).
...
- **`Bash`:** Explain intent BEFORE execution.
...
- **GAQL:** Use `sql` blocks.
- **Transparency:** Always `Read` any content written to `saved/` and display it to the user.
